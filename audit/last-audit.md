...
**상태: 관범님이 확인하기로 함. 확인 전까지 임의로 고치지 말 것.**

SKILL.md 3단계의 이체 확인 예시에는 신한은행 이체가 **525,000원**으로 적혀 있다.
그런데 `pipeline.py`의 `SHINHAN_FIXED_ITEMS` 합계는 **680,038원**이다 (16개 항목).

매달 525,000원만 들어가면 약 155,000원이 모자란다. 합성 데이터로 돌렸을 때
신한은행 통장 잔고가 음수로 떨어지는 것을 확인했다.

가능성 두 가지 — 어느 쪽인지 관범님만 판단할 수 있다:

1. 이체액을 올려야 한다 → SKILL.md 3단계 예시의 525,000원을 고친다
2. 일부 고정비가 다른 통장에서 나간다 → `SHINHAN_FIXED_ITEMS`에서 그 항목을 뺀다

**다음 회차에 관범님께 결론을 물어보고, 답을 받으면 해당 파일을 고친 뒤
이 항목을 여기서 지운다.** 답을 못 받았으면 그대로 두고 넘어간다.

참고: 실행할 때마다 `pipeline.py`가 "신한은행 통장 잔고가 음수입니다" 경고를
낼 수 있다. 위 건이 정리되기 전까지는 예상된 경고다.
